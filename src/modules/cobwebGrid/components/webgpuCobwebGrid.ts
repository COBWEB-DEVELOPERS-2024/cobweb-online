...
export async function initCobwebGrid(canvas: HTMLCanvasElement) {
    ...
    // build shape data: 4 black triangles + 4 colored inner + 4 solid squares
    const shapes: number[] = [];
    ...
    const triCells = [[8,8], [55,10], [32,32], [10,50]];
    const triColors = [0, 2, 1, 3];
    const triRotations = [0, 1, 2, 3];
    const innerTriangleFactor = 0.5;
    for (let i = 0; i < triCells.length; i++) {
        const [cx, cy] = triCells[i];
    ...
    const sqCells = [[15, 15], [48, 5], [5, 45], [30, 60]];
    const sqColors = [0, 2, 1, 3];
    for (let i = 0; i < sqCells.length; i++) {
        const [cx, cy] = sqCells[i];
    ...
    const shapeBuffer = device.createBuffer({
        size: shapes.length * 4,
        usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST
    });
    device.queue.writeBuffer(shapeBuffer, 0, new Float32Array(shapes));
